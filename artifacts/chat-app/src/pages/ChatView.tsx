...
export function ChatView({ server, identity, onServerDeleted, onUserNameChanged }: ChatViewProps) {
  ...
  const { data: serverData } = useGetServer(server.id, {
    query: { refetchInterval: 30000, initialData: server },
  });
  ...
  useEffect(() => {
    async function heartbeat() {
      ...
      queryClient.invalidateQueries({ queryKey: getGetOnlineUsersQueryKey(server.id) });
    }
  ...
  const handleSend = useCallback(async (content: string) => {
    ...
    queryClient.invalidateQueries({ queryKey: getListMessagesQueryKey(server.id) });
    ...
    <div className="flex-1 flex flex-col min-w-0">
      <div className="flex items-center justify-between px-4 py-3 border-b border-border flex-shrink-0 bg-card/50">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-foreground">{currentServer.name}</h2>
          <span className="text-xs text-muted-foreground">{currentServer.memberCount} members</span>
        </div>
        <div className="flex items-center gap-2">
          {currentServer.ownerId === userId && (
            <span className="text-xs bg-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded-full font-medium">Owner</span>
          )}
          <button
